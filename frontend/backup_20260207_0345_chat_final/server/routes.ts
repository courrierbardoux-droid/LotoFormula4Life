...
import { registerChatToken } from './chatWs';
...
export function registerRoutes(app: Express, hasDatabase: boolean = true) {
  ...
  app.get('/api/chat/contacts', requireAuth, async (req, res) => {
    try {
      ...
      // FILTRAGE : Si l'utilisateur n'est pas admin, il ne voit que les admins
      let filteredRows = userRows;
      if (user.role !== 'admin') {
        filteredRows = userRows.filter(u => u.role === 'admin');
      }

      const contacts = filteredRows.map((u) => ({ userId: u.id, username: u.username }));
      res.json({ contacts });
    } catch (e) {
  ...
  // Récupérer les infos de l'admin (pour le bouton "Contacter l'administrateur")
  app.get('/api/chat/admin-info', requireAuth, async (req, res) => {
    try {
      if (!hasDatabase) {
        // Fallback mock
        return res.json({ id: 1, username: 'ADMINISTRATEUR' });
      }
      ...
      // Chercher le premier admin (ou un admin spécifique si besoin)
      ...
      console.error('[chat/admin-info]', e);
      res.status(500).json({ error: 'Erreur serveur' });
    }
  });
  ...
  // ==========================================
  // ROUTES USER SETTINGS (Fenêtres de calcul)
  // ==========================================

  // Récupérer les préférences "Fenêtres de calcul" (par utilisateur)
  app.get('/api/user-settings/windows', requireAuth, async (req, res) => {
    try {
      ...
      const [row] = await db.select().from(userConsoleSettings).where(eq(userConsoleSettings.userId, user.id));
      if (!row) {
        return res.json({ poolWindows: null, poolWindowPresetNumbers: null });
      }
      return res.json({
  ...
  // Sauver les préférences "Fenêtres de calcul" (par utilisateur)
    ...
    try {
      ...
      if (existing) {
        await db
          ...
          .where(eq(userConsoleSettings.userId, user.id));
      } else {
      ...
      return res.status(500).json({ error: 'Erreur sauvegarde settings' });
    }
  });
